...
<div class="space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h2 class="text-2xl font-bold text-gray-800">Edit Barang Keluar</h2>
            <p class="text-sm text-gray-500 mt-1">Ubah data transaksi barang keluar</p>
        </div>
    </div>

    <!-- Form Card -->
    <div class="bg-white rounded-xl shadow-md p-6">
        <form action="{{ route('admin.barang-keluar.update', $barangKeluar->idkeluar) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="idbarang" id="idbarang" required
                        ...
                        @foreach($stocks as $stock)
                            <option value="{{ $stock->idbarang }}" {{ $barangKeluar->idbarang == $stock->idbarang ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="datetime-local" name="tanggal" id="tanggal" required
                        value="{{ \Carbon\Carbon::parse($barangKeluar->tanggal)->format('Y-m-d\TH:i') }}"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#14a2ba] focus:border-transparent">
                    @error('tanggal')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-500 mt-1">
                        <strong>Catatan:</strong> Jumlah sebelumnya: {{ $barangKeluar->qty }}. 
