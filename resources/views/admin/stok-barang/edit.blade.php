...
<div class="space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h2 class="text-2xl font-bold text-gray-800">Edit Stok Barang</h2>
            <p class="text-sm text-gray-500 mt-1">Update data stok barang</p>
        </div>
    </div>

    <!-- Form Card -->
    <div class="bg-white rounded-xl shadow-md p-6">
        <form action="{{ route('admin.stok-barang.update', $stock->idbarang) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <div class="flex gap-2">
                        <select name="rack_prefix" id="rack_prefix" required
                            ...
                            @php
                                $currentPrefix = strtoupper(substr($stock->kodebarang, 0, 2));
                            @endphp
                            <option value="1A" {{ $currentPrefix == '1A' ? 'selected' : '' }}>1A</option>
                            <option value="1B" {{ $currentPrefix == '1B' ? 'selected' : '' }}>1B</option>
                            <option value="1C" {{ $currentPrefix == '1C' ? 'selected' : '' }}>1C</option>
                            <option value="2A" {{ $currentPrefix == '2A' ? 'selected' : '' }}>2A</option>
                            <option value="2B" {{ $currentPrefix == '2B' ? 'selected' : '' }}>2B</option>
                            <option value="2C" {{ $currentPrefix == '2C' ? 'selected' : '' }}>2C</option>
                        </select>
                        <input type="text" name="kode_suffix" id="kode_suffix" required maxlength="3" pattern="[0-9]{3}"
                            value="{{ substr($stock->kodebarang, 2) }}"
                            placeholder="001"
                        ...
                        <input type="hidden" name="kodebarang" id="kodebarang" value="{{ old('kodebarang', $stock->kodebarang) }}">
                    </div>
                ...
                <div>
                    ...
                    <input type="text" name="namabarang" id="namabarang" required
                        value="{{ old('namabarang', $stock->namabarang) }}"
            ...
            <div>
                ...
                <textarea name="deskripsi" id="deskripsi" rows="3"
                    ...
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#14a2ba] focus:border-transparent @error('deskripsi') border-red-500 @enderror">{{ old('deskripsi', $stock->deskripsi) }}</textarea>
                @error('deskripsi')
                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="rack" id="rack" required
                        ...
                        <option value="1a" {{ old('rack', $stock->rack) == '1a' ? 'selected' : '' }}>Rak 1A</option>
                        <option value="1b" {{ old('rack', $stock->rack) == '1b' ? 'selected' : '' }}>Rak 1B</option>
                        <option value="1c" {{ old('rack', $stock->rack) == '1c' ? 'selected' : '' }}>Rak 1C</option>
                        <option value="2a" {{ old('rack', $stock->rack) == '2a' ? 'selected' : '' }}>Rak 2A</option>
                        <option value="2b" {{ old('rack', $stock->rack) == '2b' ? 'selected' : '' }}>Rak 2B</option>
                        <option value="2c" {{ old('rack', $stock->rack) == '2c' ? 'selected' : '' }}>Rak 2C</option>
                    </select>
                    @error('rack')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
